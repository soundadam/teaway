...
Version 0.4.1 makes the narrow passwordless helper easier to discover and
explains the one-time macOS authorization boundary before registration. The
public artifact is an immutable source tag; the Homebrew Formula builds the
executable from that source. No unnotarized prebuilt binary is published.
...
- Version strings, changelog, release notes, tag, and Formula all equal 0.4.1.
...
- A release build reports `teaway 0.4.1`.
...
- The annotated `v0.4.1` tag points to the merged and CI-tested main commit.
...
1. Update the release branch to version 0.4.1 and complete the documentation.
...
5. Create and push one annotated `v0.4.1` tag.
